...
namespace ProcessingUtility {
     ...
     * @param bandwidth Bandwidth for sdf creation inside openvdb
    ...
    ProcessForDLLDataset::ProcessForDLLDataset(const fs::path& sourceDir, const fs::path& targetDir, int kernel_size, int padding, int bandwidth, int n_min_kernel)
        : GenericDirectoryProcess(sourceDir, targetDir), kernel_size_(kernel_size), padding_(padding), bandwidth_(bandwidth), n_min_kernel_(n_min_kernel) {
    ...
    */
    void ProcessForDLLDataset::run(const std::string& subDirName)
    {
        ...
        double voxel_size = DLPP::CGALbased::calculateRecommendeVoxelsize(kernel_size_, n_min_kernel_, bandwidth_, padding_, mesh);
