...
namespace Scripts {
    ...
    /// </summary>
    void processOnJobFileTimed(ProcessingUtility::GenericDirectoryProcess* Process, fs::path job_location, int max_threads, int timeout_min) {
        ...
        for (const auto& job_name : job_list) {
            ...
            if (hasObj && hasYml) {
                ...
                try {
                    ...
                    if (!result) {
                        LOG_LEVEL("ERROR", "Processing timed out for " + subdirName);
                    }
                    else {
